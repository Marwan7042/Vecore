...
void export_weights(WildfireModel& model, const char* filepath) {
...
int main() {
    printf("--- Training Wildfire Model (Vecore CPU) ---\n");
    WildfireModel model;
    vc::optim::Adam<float> optimizer(model.parameters(), 0.001f);
    
    vc::vector<size_t> in_shape(4);
    in_shape[0] = 4; in_shape[1] = 1; in_shape[2] = 64; in_shape[3] = 64;
    ...
    export_weights(model, "wildfire_weights.bin");
    return 0;
}
